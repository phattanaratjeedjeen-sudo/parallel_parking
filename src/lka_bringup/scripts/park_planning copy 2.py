...
class ParkingController(Node):
    def __init__(self):
        ...
        # Vehicle Dimensions
        self.a = 3.005
        self.delta_max = math.radians(40.0)
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if self.current_state == 'EVALUATING':
            ...
            if yaw_diff < math.radians(1.5) and dist < 0.15:
                self.get_logger().info(f"Dist: {dist:.2f}, Yaw Diff: {math.degrees(yaw_diff):.2f} deg")
                control.steer = 0.0
                control.throttle = 0.0
                control.brake = 1.0
                ...
                if self.is_multiple_trials and self.trials_completed < self.total_trials:
                    l = self.L - (self.a + self.d_front + self.d_rear)
                    self.limit_front = self.center_target_pose[0] + 0.7*l
                    self.limit_rear  = self.center_target_pose[0] 
                    ...
                    self.current_state = 'SHUNT_FORWARD_1'
                    self.shunt_mid_x = self.limit_rear + 0.35*l
