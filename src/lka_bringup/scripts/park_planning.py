...
class ParkingController(Node):
    def __init__(self):
        ...
        self.center_target_pose = (70.95, -104.5, 0.0)       # target pose (car's center) (x, y, yaw)
    ...
    def calculate_path(self, start, target, silent=False):
        ...
        if not silent:
            self.get_logger().info(f"Minimal required parking spot length L_min = {L_min:.4f} m")
        ...
        if d_C_Einit < 1.05*d_EC_lmin:
            if not silent:
                self.get_logger().warn(f"d_C_Einit: {d_C_Einit} < d_EC_lmin: {1.05*d_EC_lmin}")
            return None
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if self.current_state == 'EVALUATING':
            ...
            if dist < 0.5 or (dist < 2.0 and dist > self.min_dist + 0.1):
